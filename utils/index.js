...
const getLoveWords = () => {
    return new Promise((resolve, reject) => {
        const apiKey = '5273d9cbc195a165dafa72944acaf7c5';
...
const getValueByMonth = () => {
    ...
    if (jsonData[monthsList[currentMonth]] !== "None" && jsonData[monthsList[currentMonth]] !== "NULL") {
        ...
        let lastValidMonth = currentMonth - 1;
        while (lastValidMonth > 0 && (jsonData[monthsList[lastValidMonth]] === "None" || jsonData[monthsList[lastValidMonth]] === "NULL")) {
            lastValidMonth--;
        }
        
        if (lastValidMonth > 0) {
            const value = jsonData[monthsList[lastValidMonth]];
            if (currentDay <= value) {
                return `${lastValidMonth}.${value}，预计于 ${value - currentDay} 天后到来！`;
            } else {
                return `${lastValidMonth}.${value}，较上月已推迟 ${currentDay - value + 2} 天！`;
            }
        } else {
            return '暂无生理期记录';
        }
